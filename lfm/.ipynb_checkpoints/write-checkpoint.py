...
class VanillaWriter:
    ...
    def __init__(self, fn, name, dtype, shape):
        if len(shape) == 3:
            self.write_function = self._write_frame
        elif len(shape) == 4:
            self.write_function =self._write_vol_frame
        else:
            raise ValueError("final output shape not supported")
        self.h5f = h5py.File(fn, "a")
        self.ds = self.h5f.create_dataset(name=name, dtype=dtype, shape=shape)


    def _write_vol_frame(self, frame, iVol, iPlane):
        self.ds[iVol, iPlane] = frame

    def _write_frame(self, frame, iFrame):
        self.ds[iFrame] = frame

    def write_frame(self,frame, *args):
        self.write_function(frame, *args)
...
class ParallelCompressedWriter:
    ...
    def write_chunk(self, frame, chunk_offsets):
        ...
        assert frame.shape == self.chunk_shape  # Ensure the chunk shape matches
    ...
    def _write_frame(self, frame, iFrame):
        self.write_chunk(frame[None], (iFrame, 0, 0))
    ...
    def run_(self):
        ...
        with h5py.File(self.fn, "a") as f:
            ...
            def callback(arr, meta):
                if meta is None:
                    return
                did.write_direct_chunk(meta, arr.ravel())  # Maintain chunked writing using meta offsets
