...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', help='Config file name', default='gui_lfm_defaults.yml')
    ...
    def button_callback(name):
        ...
        #     threaded(opm.start_ortho_preview)(di.to_dict())
            ...
            try:
                lfm.acquire_timelapse(di.to_dict())
            except: 
                logger.error('Error during acquisition')
                logger.error(traceback.format_exc())
                lfm.point()
        elif name == "abort":
            lfm.interrupt_flag = True
        elif name == "mosaic":
            lfm.cam.set_trigger(False)
            lfm.cam.preview_mosaic([100, 100], [50, 50])
    ...
    lfm = LFM(
        rate=params_dict["daq"]["rate"],
        aochans=f"{params_dict['daq']['device']}/{params_dict['daq']['ao_channels']}",
        cam_trigger_line=params_dict["daq"]["do_port"],
        shutter_line=params_dict["daq"]["shutter_line"],
        cam_gain=params_dict["camera"]["gain"],
    )
